...
function register() {
	...
	$.ajax({
		...
		success: function(msg){
			if(msg.info=="下一步"){
				window.location.href="../showUserActive.action";
			}
			if(msg.info=="注册成功,请登录"){
				layer.msg(msg.info);
				setTimeout(function(){
					window.location.href="../showUserLogin.action";
				},2000);
			}
...
function active(){
	...
	$.ajax({
		...
		success: function(msg){
			if(msg.info=="激活成功"){
				alert("激活成功,请登录");
				window.location.href="../showUserLogin";
			}else if(msg.info=="激活码失效，请重新注册"){
				alert("激活码失效，请重新注册");
				window.location.href="../showUserRegister.action";
